...
- **Netzkosten** kommen aus den offiziellen Netzbetreiber-Preisblättern, cross-validiert
  gegen die **Verordnung** (SNE-V 2018 – Novelle 2026, BGBl. II Nr. 305/2025).
...
- **`quelle`** — die Preisblatt-URL des Netzbetreibers,
- **`gueltig_ab`** — ab wann der Wert gilt (z. B. Energienetze Steiermark `2026-04-01`),
- und über das MANIFEST den **Verweis auf die SNE-V-BGBl-Nummer** (BGBl. II Nr. 305/2025)
  als verordnungsseitigen Cross-Check.
...
- **Audit-Pfad.** Ergebnis → Rechenweg-Schritte → erhobene Rohgröße → `quelle`-URL
  ...
  (SNE-V BGBl. II Nr. 305/2025; EAG-/Elektrizitätsabgabe-Verordnungen). Dieser Pfad ist für
...
1. `data/tariffs/MANIFEST.json` öffnen → `generated_at`, `price_basis = netto_listenpreis`,
   `provider_coverage 65/65`, `provenance` nennt First-Party-Scrape + Preisblatt-PDFs.
...
3. Rechenweg aus §2.3 von Hand auf diesen Tarif anwenden → Brutto-Jahreskosten
   reproduzieren.
4. Einen Netzbereich in `netzkosten.json` wählen → `quelle` (Preisblatt-URL) öffnen,
   `netznutzung_arbeitspreis_ct_kwh` bestätigen, gegen SNE-V (BGBl. II Nr. 305/2025)
   gegenprüfen.
