...
# Branding/Bevollmächtigten-Daten sind projektspezifisch zu setzen. Im publizierten
# Open-Source-Kern stehen neutrale Platzhalter; ein Deployment überschreibt sie via
# der Parameter von :func:`generate_switching_pdf` (bevollmaechtigter_*).
_DEFAULT_BRAND_TITLE = "Energie-Agent"
_DEFAULT_BEVOLLMAECHTIGTER_NAME = "[Bevollmächtigter]"
_DEFAULT_BEVOLLMAECHTIGTER_KONTAKT = "[Kontakt-E-Mail / Web]"


class _BrandedPDF(FPDF):
    """PDF mit projektspezifischem Branding (Platzhalter im Open-Source-Kern)."""

    brand_title: str = _DEFAULT_BRAND_TITLE

    def header(self) -> None:
        self.set_font("Helvetica", "B", 10)
        self.set_text_color(22, 163, 74)  # green-dark
        self.cell(0, 8, self.brand_title, align="R")
        self.ln(12)
...
def _add_field(pdf: _BrandedPDF, label: str, value: str) -> None:
...
def generate_switching_pdf(
    *,
    ...
    bevollmaechtigter_name: str = _DEFAULT_BEVOLLMAECHTIGTER_NAME,
    bevollmaechtigter_kontakt: str = _DEFAULT_BEVOLLMAECHTIGTER_KONTAKT,
    bevollmaechtigter_email: str = "",
    brand_title: str = _DEFAULT_BRAND_TITLE,
) -> Path:
    """Vollmacht als PDF generieren.

    Vollmacht an den (projektspezifisch zu setzenden) Bevollmächtigten — nicht an
    den neuen Lieferanten. Basiert auf dem OGH-orientierten Modell. Die
    Bevollmächtigten-/Branding-Daten kommen aus den ``bevollmaechtigter_*``- bzw.
    ``brand_title``-Parametern; im publizierten Kern stehen neutrale Platzhalter.
    """
    pdf = _BrandedPDF()
    pdf.brand_title = brand_title
    pdf.add_page()
    ...
    pdf.set_font("Helvetica", "B", 10)
    pdf.set_text_color(30, 41, 59)
    pdf.cell(0, 7, bevollmaechtigter_name)
    pdf.ln(7)
    pdf.set_font("Helvetica", "", 9)
    pdf.set_text_color(100, 116, 139)
    pdf.cell(0, 6, bevollmaechtigter_kontakt)
    pdf.ln(10)
    ...
    pdf.multi_cell(0, 4.5,
        ...
        + (
            f"(auch per E-Mail an {bevollmaechtigter_email}) "
            if bevollmaechtigter_email
            else "(auch per E-Mail an den Bevollmächtigten) "
        )
        + "möglich und wird unverzüglich wirksam."
    )
    pdf.ln(6)
    ...
    pdf.multi_cell(0, 5,
        ...
        f"von {user_name or '[Name]'} digital erteilt und bestätigt. "
    ...
    # Speichern
    tmpdir = tempfile.mkdtemp(prefix="energietools_pdf_")
    out_path = Path(tmpdir) / "vollmacht.pdf"
    pdf.output(str(out_path))
    log.info("Vollmacht erstellt: %s", out_path)
    return out_path
